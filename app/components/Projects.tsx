...
//console.log("Tag Names:", JSON.stringify(projects[0]?.acf?.tech_stacks?.map((t: any) => t.name || t)));
...
//console.log(`Project ${project.id} Embedded Data:`, project._embedded?.['wp:term']);
                console.log("Full Project Object:", project);
           // const projectData = project._embedded?.['wp:term']?.[0] || project._embedded?.['wp:term']?.[1] || [];;
            const projectData = project._embedded?.['wp:term']?.[0] || [];
            ...
            console.log(`Project ${project.id} All Terms:`, project._embedded?.['wp:term']?.flat().map((t: any) => t.taxonomy));
             
            return(
                    <article key={project.id} className="project-card">
                        {projectData.map((tag: any) => (
